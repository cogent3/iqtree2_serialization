...
json Params::to_json() const {
    ...
    j["fai"] = this->fai;  //bool
    j["memCheck"] = this->memCheck;  //bool
    j["stableSplitThreshold"] = this->stableSplitThreshold;  // double
    j["adaptPertubation"] = this->adaptPertubation;  // bool
    j["testAlpha"] = this->testAlpha;  // bool
    j["opt_gammai"] = this->opt_gammai;  // bool
    j["opt_gammai_fast"] = this->opt_gammai_fast;  // bool
    j["opt_gammai_keep_bran"] = this->opt_gammai_keep_bran;  // bool
    j["testAlphaEpsAdaptive"] = this->testAlphaEpsAdaptive;  // bool
    j["randomAlpha"] = this->randomAlpha;  // bool
    j["testAlphaEps"] = this->testAlphaEps;  // double
    j["exh_ai"] = this->exh_ai;  // bool
    j["alpha_invar_file"] = std::string(this->alpha_invar_file);  // char*
    j["tabu"] = this->tabu;  // bool
    j["five_plus_five"] = this->five_plus_five;  // bool
    j["fixStableSplits"] = this->fixStableSplits;  // bool
    j["numSupportTrees"] = this->numSupportTrees;  // int
    j["maxCandidates"] = this->maxCandidates;  // int
    j["numInitTrees"] = this->numInitTrees;  // int
    j["sprDist"] = this->sprDist;  // int
    j["sankoff_cost_file"] = std::string(this->sankoff_cost_file);  // char*
    j["numNNITrees"] = this->numNNITrees;  // int
    j["popSize"] = this->popSize;  // int
    j["speednni"] = this->speednni;  // bool
    j["initPS"] = this->initPS;  // double
    j["modelEps"] = this->modelEps;  // double
    j["modelfinder_eps"] = this->modelfinder_eps;  // double
    j["snni"] = this->snni;  // bool
    ::to_json(j["nni_type"], this->nni_type); // NNI_Type enum
    ::to_json(j["ls_var_type"], this->ls_var_type); // LEAST_SQUARE_VAR_TYPE enum
    j["nniThresHold"] = this->nniThresHold;  // double
    j["nni5"] = this->nni5;  // bool
    j["nni5_num_eval"] = this->nni5_num_eval;  // int
    j["brlen_num_traversal"] = this->brlen_num_traversal;  // int
    j["numSmoothTree"] = this->numSmoothTree;  // int
    j["leastSquareBranch"] = this->leastSquareBranch;  // bool
    j["manuel_analytic_approx"] = this->manuel_analytic_approx;  // bool
    j["pars_branch_length"] = this->pars_branch_length;  // bool
    j["bayes_branch_length"] = this->bayes_branch_length;  // bool
    j["leastSquareNNI"] = this->leastSquareNNI;  // bool
    j["loglh_epsilon"] = this->loglh_epsilon;  // double
    j["reinsert_par"] = this->reinsert_par;  // bool
    j["bestStart"] = this->bestStart;  // bool
    j["maxtime"] = this->maxtime;  // double
    j["parbran"] = this->parbran;  // bool
    j["pll"] = this->pll;  // bool
    j["unsuccess_iteration"] = this->unsuccess_iteration;  // int
    j["binary_aln_file"] = std::string(this->binary_aln_file);  // char*
    j["speedup_iter"] = this->speedup_iter;  // int
    j["startCPUTime"] = this->startCPUTime;  // double
    j["start_real_time"] = this->start_real_time;  // double
    j["iteration_multiple"] = this->iteration_multiple;  // int
    j["user_file"] = std::string(this->user_file);  // char*
    ::to_json(j["start_tree"], this->start_tree); // START_TREE_TYPE enum
    j["start_tree_subtype_name"] = this->start_tree_subtype_name;  // std::string
    j["modelfinder_ml_tree"] = this->modelfinder_ml_tree;  // bool
    j["final_model_opt"] = this->final_model_opt;  // bool
    j["constraint_tree_file"] = std::string(this->constraint_tree_file);  // char*
    j["out_prefix"] = std::string(this->out_prefix);  // char*
    j["aln_file"] = std::string(this->aln_file);  // char*
    j["phylip_sequential_format"] = this->phylip_sequential_format;  // bool
    ::to_json(j["symtest"], this->symtest); // SymTest enum
    j["symtest_only"] = this->symtest_only;  // bool
    j["symtest_remove"] = this->symtest_remove;  // int
    j["symtest_keep_zero"] = this->symtest_keep_zero;  // bool
    j["symtest_type"] = this->symtest_type;  // int
    j["symtest_pcutoff"] = this->symtest_pcutoff;  // double
    j["symtest_stat"] = this->symtest_stat;  // double
    j["symtest_shuffle"] = this->symtest_shuffle;  // int
    j["treeset_file"] = this->treeset_file;  // string
    j["topotest_replicates"] = this->topotest_replicates;  // int
    j["topotest_optimize_model"] = this->topotest_optimize_model;  // bool
    j["do_weighted_test"] = this->do_weighted_test;  // bool
    j["do_au_test"] = this->do_au_test;  // bool
    j["partition_file"] = std::string(this->partition_file);  // char*
    j["partition_type"] = this->partition_type;  // int
    ::to_json(j["partition_merge"], this->partition_merge); // PartitionMerge enum
    j["partfinder_rcluster"] = this->partfinder_rcluster;  // double
    j["partfinder_rcluster_max"] = this->partfinder_rcluster_max;  // size_t
    j["merge_models"] = this->merge_models;  // string
    j["merge_rates"] = this->merge_rates;  // string
    j["partfinder_log_rate"] = this->partfinder_log_rate;  // bool
    j["remove_empty_seq"] = this->remove_empty_seq;  // bool
    j["terrace_aware"] = this->terrace_aware;  // bool
    j["terrace_analysis_tphast"] = this->terrace_analysis_tphast;  // bool
    j["terrace_check"] = this->terrace_check;  // bool
    j["terrace_analysis"] = this->terrace_analysis;  // bool
    j["pr_ab_matrix"] = std::string(this->pr_ab_matrix);  // const char*
    j["print_terrace_trees"] = this->print_terrace_trees;  // bool
    j["print_induced_trees"] = this->print_induced_trees;  // bool
    j["print_pr_ab_matrix"] = this->print_pr_ab_matrix;  // bool
    j["print_m_overlap"] = this->print_m_overlap;  // bool
    j["terrace_query_set"] = std::string(this->terrace_query_set);  // char*
    j["terrace_stop_intermediate_num"] = this->terrace_stop_intermediate_num;  // int
    j["terrace_stop_terrace_trees_num"] = this->terrace_stop_terrace_trees_num;  // int
    j["terrace_stop_time"] = this->terrace_stop_time;  // double
    j["terrace_non_stop"] = this->terrace_non_stop;  // bool
    j["terrace_remove_m_leaves"] = this->terrace_remove_m_leaves;  // int
    j["terrace_print_lim"] = this->terrace_print_lim;  // int
    j["matrix_order"] = this->matrix_order;  // bool
    j["gen_all_NNI"] = this->gen_all_NNI;  // bool
    j["sequence_type"] = std::string(this->sequence_type);  // char*
    j["aln_output"] = std::string(this->aln_output);  // char*
    j["siteLL_file"] = std::string(this->siteLL_file);  // char*
    j["gap_masked_aln"] = std::string(this->gap_masked_aln);  // char*
...
//TODO     j["output_format"] = this->output_format;  // FileFormat
    ...
    j["print_aln_info"] = this->print_aln_info;  // bool
    j["out_file"] = std::string(this->out_file);  // char*
    j["sub_size"] = this->sub_size;  // int
    j["min_size"] = this->min_size;  // int
    j["step_size"] = this->step_size;  // int
    j["pd_proportion"] = this->pd_proportion;  // double
    j["min_proportion"] = this->min_proportion;  // double
    j["step_proportion"] = this->step_proportion;  // double
    j["sample_size"] = this->sample_size;  // int
    j["find_all"] = this->find_all;  // bool
    ::to_json(j["tree_gen"], this->tree_gen); // TreeGenType enum
    ...
    j["pdtaxa_file"] = std::string(this->pdtaxa_file);  // char*
    j["areas_boundary_file"] = std::string(this->areas_boundary_file);  // char*
    j["boundary_modifier"] = this->boundary_modifier;  // double
    j["dist_file"] = std::string(this->dist_file);  // char*
    j["compute_obs_dist"] = this->compute_obs_dist;  // bool
    j["compute_jc_dist"] = this->compute_jc_dist;  // bool
    j["experimental"] = this->experimental;  // bool
    j["compute_ml_dist"] = this->compute_ml_dist;  // bool
    j["compute_ml_tree"] = this->compute_ml_tree;  // bool
    j["compute_ml_tree_only"] = this->compute_ml_tree_only;  // bool
    j["budget_file"] = std::string(this->budget_file);  // char*
    j["overlap"] = this->overlap;  // int
    j["repeated_time"] = this->repeated_time;  // int
    j["nr_output"] = this->nr_output;  // int
    j["intype"] = this->intype;  // InputType
    j["budget"] = this->budget;  // int
    j["min_budget"] = this->min_budget;  // int
    j["step_budget"] = this->step_budget;  // int
    j["root"] = std::string(this->root);  // const char*
    j["is_rooted"] = this->is_rooted;  // bool
    j["root_move_dist"] = this->root_move_dist;  // int
    j["root_find"] = this->root_find;  // bool
    j["root_test"] = this->root_test;  // bool
    j["min_len"] = this->min_len;  // double
    j["mean_len"] = this->mean_len;  // double
    j["max_len"] = this->max_len;  // double
    j["num_zero_len"] = this->num_zero_len;  // int
    j["ran_seed"] = this->ran_seed;  // int
    j["run_time"] = this->run_time;  // double
    j["pd_limit"] = this->pd_limit;  // int
    j["calc_pdgain"] = this->calc_pdgain;  // bool
    j["multi_tree"] = this->multi_tree;  // bool
    j["second_tree"] = std::string(this->second_tree);  // char*
    j["support_tag"] = std::string(this->support_tag);  // char*
    j["site_concordance"] = this->site_concordance;  // int
    j["ancestral_site_concordance"] = this->ancestral_site_concordance;  // int
    j["site_concordance_partition"] = this->site_concordance_partition;  // bool
    j["print_cf_quartets"] = this->print_cf_quartets;  // bool
    j["print_df1_trees"] = this->print_df1_trees;  // bool
    j["internode_certainty"] = this->internode_certainty;  // int
    j["second_align"] = std::string(this->second_align);  // char*
    ::to_json(j["consensus_type"], this->consensus_type); // ConsensusType enum
    j["tree_weight_file"] = std::string(this->tree_weight_file);  // char*
    j["find_pd_min"] = this->find_pd_min;  // bool
    j["endemic_pd"] = this->endemic_pd;  // bool
    j["exclusive_pd"] = this->exclusive_pd;  // bool
    j["complement_area"] = std::string(this->complement_area);  // char*
    j["branch_cluster"] = this->branch_cluster;  // int
    j["taxa_order_file"] = std::string(this->taxa_order_file);  // char*
    j["scaling_factor"] = this->scaling_factor;  // double
    j["binary_programming"] = this->binary_programming;  // bool
    j["test_input"] = this->test_input;  // TestType
    j["tree_burnin"] = this->tree_burnin;  // int
    j["tree_max_count"] = this->tree_max_count;  // int
    j["split_threshold"] = this->split_threshold;  // double
    j["split_threshold_str"] = std::string(this->split_threshold_str);  // char*
    j["split_weight_threshold"] = this->split_weight_threshold;  // double
    j["collapse_zero_branch"] = this->collapse_zero_branch;  // bool
    j["split_weight_summary"] = this->split_weight_summary;  // double
    j["quad_programming"] = this->quad_programming;  // bool
    j["tree_spr"] = this->tree_spr;  // bool
    j["nexus_output"] = this->nexus_output;  // bool
    j["k_representative"] = this->k_representative;  // int
    j["p_delete"] = this->p_delete;  // double
    j["min_iterations"] = this->min_iterations;  // int
    j["max_iterations"] = this->max_iterations;  // int
    ::to_json(j["stop_condition"], this->stop_condition); // STOP_CONDITION enum
    j["stop_confidence"] = this->stop_confidence;  // double
    j["num_param_iterations"] = this->num_param_iterations;  // int
    j["num_runs"] = this->num_runs;  // int
    j["model_name"] = this->model_name;  // string
    j["model_name_init"] = std::string(this->model_name_init);  // char*
    j["model_opt_steps"] = this->model_opt_steps;  // int
    j["model_set"] = this->model_set;  // string
    j["model_extra_set"] = std::string(this->model_extra_set);  // char*
    j["model_subset"] = std::string(this->model_subset);  // char*
    j["state_freq_set"] = std::string(this->state_freq_set);  // char*
    j["ratehet_set"] = this->ratehet_set;  // string
    j["score_diff_thres"] = this->score_diff_thres;  // double
    j["model_def_file"] = std::string(this->model_def_file);  // char*
    j["modelomatic"] = this->modelomatic;  // bool
    j["model_test_again"] = this->model_test_again;  // bool
    j["model_test_and_tree"] = this->model_test_and_tree;  // short int
    j["model_test_separate_rate"] = this->model_test_separate_rate;  // bool
    j["optimize_mixmodel_weight"] = this->optimize_mixmodel_weight;  // bool
    j["num_mixlen"] = this->num_mixlen;  // int
    j["optimize_rate_matrix"] = this->optimize_rate_matrix;  // bool
    j["store_trans_matrix"] = this->store_trans_matrix;  // bool
    ::to_json(j["freq_type"], this->freq_type); // StateFreqType enum
    j["keep_zero_freq"] = this->keep_zero_freq;  // bool
    j["min_state_freq"] = this->min_state_freq;  // double
    j["num_rate_cats"] = this->num_rate_cats;  // int
    j["min_rate_cats"] = this->min_rate_cats;  // int
    j["max_rate_cats"] = this->max_rate_cats;  // int
    j["gamma_shape"] = this->gamma_shape;  // double
    j["min_gamma_shape"] = this->min_gamma_shape;  // bool
    ...
    j["optimize_alg_treeweight"] = this->optimize_alg_treeweight;  // string
    j["optimize_from_given_params"] = this->optimize_from_given_params;  // bool
    j["fixed_branch_length"] = this->fixed_branch_length;  // int
    j["min_branch_length"] = this->min_branch_length;  // double
    j["max_branch_length"] = this->max_branch_length;  // double
    ::to_json(j["iqp_assess_quartet"], this->iqp_assess_quartet); // IQP_ASSESS_QUARTET enum
    j["iqp"] = this->iqp;  // bool
    j["gurobi_format"] = this->gurobi_format;  // bool
    j["gurobi_threads"] = this->gurobi_threads;  // bool
    j["num_bootstrap_samples"] = this->num_bootstrap_samples;  // int
    j["bootstrap_spec"] = std::string(this->bootstrap_spec);  // char*
    j["transfer_bootstrap"] = this->transfer_bootstrap;  // int
    j["subsampling"] = this->subsampling;  // int
    j["subsampling_seed"] = this->subsampling_seed;  // int
    j["write_intermediate_trees"] = this->write_intermediate_trees;  // bool
    j["writeDistImdTrees"] = this->writeDistImdTrees;  // bool
    j["write_candidate_trees"] = this->write_candidate_trees;  // int
    j["rf_dist_mode"] = this->rf_dist_mode;  // int
    j["rf_same_pair"] = this->rf_same_pair;  // bool
    j["normalize_tree_dist"] = this->normalize_tree_dist;  // bool
    j["mvh_site_rate"] = this->mvh_site_rate;  // bool
    j["rate_mh_type"] = this->rate_mh_type;  // bool
    j["discard_saturated_site"] = this->discard_saturated_site;  // bool
    j["mean_rate"] = this->mean_rate;  // double
    j["aLRT_threshold"] = this->aLRT_threshold;  // int
    j["aLRT_replicates"] = this->aLRT_replicates;  // int
    j["aLRT_test"] = this->aLRT_test;  // bool
    j["aBayes_test"] = this->aBayes_test;  // bool
    j["localbp_replicates"] = this->localbp_replicates;  // int
    ::to_json(j["SSE"], this->SSE); // LikelihoodKernel enum
    j["lk_safe_scaling"] = this->lk_safe_scaling;  // bool
    j["numseq_safe_scaling"] = this->numseq_safe_scaling;  // int
    j["kernel_nonrev"] = this->kernel_nonrev;  // bool
    ::to_json(j["print_site_lh"], this->print_site_lh); // SiteLoglType enum
    j["print_partition_lh"] = this->print_partition_lh;  // bool
    ::to_json(j["print_site_prob"], this->print_site_prob); // SiteLoglType enum
    ::to_json(j["print_ancestral_sequence"], this->print_ancestral_sequence); // AncestralSeqType enum
    j["min_ancestral_prob"] = this->min_ancestral_prob;  // double
    ::to_json(j["print_site_state_freq"], this->print_site_state_freq); // SiteFreqType enum
    j["print_site_rate"] = this->print_site_rate;  // int
    j["print_trees_site_posterior"] = this->print_trees_site_posterior;  // int
    j["print_tree_lh"] = this->print_tree_lh;  // bool
    j["print_branch_lengths"] = this->print_branch_lengths;  // bool
    j["nni_lh"] = this->nni_lh;  // bool
    j["speedUpFromIter"] = this->speedUpFromIter;  // int
    j["lambda"] = this->lambda;  // double
    j["speed_conf"] = this->speed_conf;  // double
    j["new_heuristic"] = this->new_heuristic;  // bool
    j["whtest_simulations"] = this->whtest_simulations;  // double
    j["whtest_delta"] = this->whtest_delta;  // double
    j["whtest_delta_quantile"] = this->whtest_delta_quantile;  // double
    j["whtest_p_value"] = this->whtest_p_value;  // double
    j["mcat_type"] = this->mcat_type;  // int
    j["rate_file"] = std::string(this->rate_file);  // char*
    j["ngs_file"] = std::string(this->ngs_file);  // char*
    j["ngs_mapped_reads"] = std::string(this->ngs_mapped_reads);  // char*
    j["ngs_ignore_gaps"] = this->ngs_ignore_gaps;  // bool
    j["do_pars_multistate"] = this->do_pars_multistate;  // bool
    j["gene_pvalue_file"] = std::string(this->gene_pvalue_file);  // char*
    j["gene_scale_factor"] = this->gene_scale_factor;  // double
    j["gene_pvalue_loga"] = this->gene_pvalue_loga;  // bool
    j["ncbi_taxid"] = this->ncbi_taxid;  // int
    j["ncbi_taxon_level"] = std::string(this->ncbi_taxon_level);  // char*
    j["ncbi_ignore_level"] = std::string(this->ncbi_ignore_level);  // char*
    j["ncbi_names_file"] = std::string(this->ncbi_names_file);  // char*
    j["eco_dag_file"] = std::string(this->eco_dag_file);  // char*
    j["eco_detail_file"] = std::string(this->eco_detail_file);  // char*
    j["eco_type"] = std::string(this->eco_type);  // char*
    j["k_percent"] = this->k_percent;  // int
    j["diet_min"] = this->diet_min;  // int
    j["diet_max"] = this->diet_max;  // int
    j["diet_step"] = this->diet_step;  // int
    j["eco_run"] = this->eco_run;  // int
    j["eco_weighted"] = this->eco_weighted;  // bool
    j["upper_bound"] = this->upper_bound;  // bool
    j["upper_bound_NNI"] = this->upper_bound_NNI;  // bool
    j["upper_bound_frac"] = this->upper_bound_frac;  // double
    j["gbo_replicates"] = this->gbo_replicates;  // int
    j["ufboot_epsilon"] = this->ufboot_epsilon;  // double
    j["check_gbo_sample_size"] = this->check_gbo_sample_size;  // bool
    j["use_rell_method"] = this->use_rell_method;  // bool
    j["use_elw_method"] = this->use_elw_method;  // bool
    j["use_weighted_bootstrap"] = this->use_weighted_bootstrap;  // bool
    j["use_max_tree_per_bootstrap"] = this->use_max_tree_per_bootstrap;  // bool
    j["max_candidate_trees"] = this->max_candidate_trees;  // int
    j["distinct_trees"] = this->distinct_trees;  // bool
    j["online_bootstrap"] = this->online_bootstrap;  // bool
    j["min_correlation"] = this->min_correlation;  // double
    j["step_iterations"] = this->step_iterations;  // int
    j["print_ufboot_trees"] = this->print_ufboot_trees;  // double
    j["jackknife_prop"] = this->jackknife_prop;  // double
    j["robust_phy_keep"] = this->robust_phy_keep;  // double
    j["robust_median"] = this->robust_median;  // bool
    j["estimate_nni_cutoff"] = this->estimate_nni_cutoff;  // bool
    j["nni_cutoff"] = this->nni_cutoff;  // double
    j["nni_sort"] = this->nni_sort;  // bool
    j["testNNI"] = this->testNNI;  // bool
    j["approximate_nni"] = this->approximate_nni;  // bool
    j["do_compression"] = this->do_compression;  // bool
    j["avh_test"] = this->avh_test;  // int
    j["bootlh_test"] = this->bootlh_test;  // int
    j["bootlh_partitions"] = std::string(this->bootlh_partitions);  // char*
    j["numeric_precision"] = this->numeric_precision;  // int
    j["site_freq_file"] = std::string(this->site_freq_file);  // char*
    j["tree_freq_file"] = std::string(this->tree_freq_file);  // char*
    j["num_threads"] = this->num_threads;  // int
    j["num_threads_max"] = this->num_threads_max;  // int
    j["openmp_by_model"] = this->openmp_by_model;  // bool
    ::to_json(j["model_test_criterion"], this->model_test_criterion); // ModelTestCriterion enum
    j["model_test_sample_size"] = this->model_test_sample_size;  // int
    j["root_state"] = std::string(this->root_state);  // char*
    j["print_bootaln"] = this->print_bootaln;  // bool
    j["print_boot_site_freq"] = this->print_boot_site_freq;  // bool
    j["print_subaln"] = this->print_subaln;  // bool
    j["print_partition_info"] = this->print_partition_info;  // bool
    j["print_conaln"] = this->print_conaln;  // bool
    j["link_alpha"] = this->link_alpha;  // bool
    j["link_model"] = this->link_model;  // bool
    j["model_joint"] = std::string(this->model_joint);  // char*
    j["count_trees"] = this->count_trees;  // bool
    j["pomo"] = this->pomo;  // bool
    j["pomo_random_sampling"] = this->pomo_random_sampling;  // bool
    j["pomo_pop_size"] = this->pomo_pop_size;  // int
    ::to_json(j["lh_mem_save"], this->lh_mem_save); // LhMemSave enum
    j["buffer_mem_save"] = this->buffer_mem_save;  // bool
    j["max_mem_size"] = this->max_mem_size;  // double
    j["print_splits_file"] = this->print_splits_file;  // bool
    j["print_splits_nex_file"] = this->print_splits_nex_file;  // bool
    j["ignore_identical_seqs"] = this->ignore_identical_seqs;  // bool
    j["write_init_tree"] = this->write_init_tree;  // bool
    j["write_branches"] = this->write_branches;  // bool
    j["freq_const_patterns"] = std::string(this->freq_const_patterns);  // char*
    j["no_rescale_gamma_invar"] = this->no_rescale_gamma_invar;  // bool
    j["compute_seq_identity_along_tree"] = this->compute_seq_identity_along_tree;  // bool
    j["compute_seq_composition"] = this->compute_seq_composition;  // bool
    j["ignore_checkpoint"] = this->ignore_checkpoint;  // bool
    j["lmap_num_quartets"] = this->lmap_num_quartets;  // int64_t
    j["lmap_cluster_file"] = std::string(this->lmap_cluster_file);  // char*
    j["checkpoint_dump_interval"] = this->checkpoint_dump_interval;  // int
    j["print_lmap_quartet_lh"] = this->print_lmap_quartet_lh;  // bool
    j["force_unfinished"] = this->force_unfinished;  // bool
    j["print_all_checkpoints"] = this->print_all_checkpoints;  // bool
    j["suppress_output_flags"] = this->suppress_output_flags;  // int
    ::to_json(j["matrix_exp_technique"], this->matrix_exp_technique); // MatrixExpTechnique enum
    j["ufboot2corr"] = this->ufboot2corr;  // bool
    j["u2c_nni5"] = this->u2c_nni5;  // bool
    j["dating_method"] = this->dating_method;  // string
    j["dating_options"] = this->dating_options;  // string
    j["date_file"] = this->date_file;  // string
    j["date_tip"] = this->date_tip;  // string
    j["date_root"] = this->date_root;  // string
    j["date_with_outgroup"] = this->date_with_outgroup;  // bool
    j["date_debug"] = this->date_debug;  // bool
    j["date_replicates"] = this->date_replicates;  // int
    j["clock_stddev"] = this->clock_stddev;  // double
    j["date_outlier"] = this->date_outlier;  // double
    j["suppress_list_of_sequences"] = this->suppress_list_of_sequences;  // double
    j["suppress_zero_distance_warnings"] = this->suppress_zero_distance_warnings;  // double
    j["suppress_duplicate_sequence_warnings"] = this->suppress_duplicate_sequence_warnings;  // bool
    j["alisim_active"] = this->alisim_active;  // bool
    j["alisim_inference_mode"] = this->alisim_inference_mode;  // bool
    j["alisim_no_copy_gaps"] = this->alisim_no_copy_gaps;  // bool
    j["original_params"] = this->original_params;  // string
    j["alisim_output_filename"] = this->alisim_output_filename;  // string
    j["alisim_sequence_length"] = this->alisim_sequence_length;  // int
    j["alisim_dataset_num"] = this->alisim_dataset_num;  // int
    j["alisim_ancestral_sequence_aln_filepath"] = std::string(this->alisim_ancestral_sequence_aln_filepath);  // char*
    j["alisim_ancestral_sequence_name"] = this->alisim_ancestral_sequence_name;  // string
    j["alisim_max_rate_categories_for_applying_caching"] = this->alisim_max_rate_categories_for_applying_caching;  // int
...
//TODO    j["alisim_deletion_distribution"] = this->alisim_deletion_distribution;  // IndelDistribution
    ...
    j["alisim_no_export_sequence_wo_gaps"] = this->alisim_no_export_sequence_wo_gaps;  // bool
    j["alisim_mixture_at_sub_level"] = this->alisim_mixture_at_sub_level;  // bool
    j["alisim_branch_scale"] = this->alisim_branch_scale;  // double
    j["alisim_single_output"] = this->alisim_single_output;  // bool
    ::to_json(j["alisim_rate_heterogeneity"], this->alisim_rate_heterogeneity); // ASSIGNMENT_TYPE enum
    ::to_json(j["alisim_stationarity_heterogeneity"], this->alisim_stationarity_heterogeneity); // ASSIGNMENT_TYPE enum
    j["tmp_data_filename"] = this->tmp_data_filename;  // string
    j["keep_seq_order"] = this->keep_seq_order;  // bool
    j["delete_output"] = this->delete_output;  // bool
    j["rebuild_indel_history_param"] = this->rebuild_indel_history_param;  // double
    j["mem_limit_factor"] = this->mem_limit_factor;  // double
    j["indel_rate_variation"] = this->indel_rate_variation;  // bool
    ::to_json(j["alisim_openmp_alg"], this->alisim_openmp_alg); // ALI_OPENMP_ALG enum
    j["no_merge"] = this->no_merge;  // bool
    j["include_pre_mutations"] = this->include_pre_mutations;  // bool
...
void Params::setDefault(){
    this->tree_gen = NONE;
    this->user_file = NULL;
    this->constraint_tree_file = NULL;
    this->opt_gammai = true;
    this->opt_gammai_fast = false;
    this->opt_gammai_keep_bran = false;
    this->testAlphaEpsAdaptive = false;
    this->randomAlpha = false;
    this->testAlphaEps = 0.1;
    this->exh_ai = false;
    this->alpha_invar_file = NULL;
    this->out_prefix = NULL;
    this->out_file = NULL;
    this->sub_size = 4;
    this->pd_proportion = 0.0;
    this->min_proportion = 0.0;
    this->step_proportion = 0.01;
    this->min_size = 0;
    this->step_size = 1;
    this->find_all = false;
    this->run_mode = DETECTED;
    this->detected_mode = DETECTED;
    this->param_file = NULL;
    this->initial_file = NULL;
    this->initial_area_file = NULL;
    this->pdtaxa_file = NULL;
    this->areas_boundary_file = NULL;
    this->boundary_modifier = 1.0;
    this->dist_file = NULL;
    this->compute_obs_dist = false;
    this->compute_jc_dist = true;
    this->experimental = true;
    this->compute_ml_dist = true;
    this->compute_ml_tree = true;
    this->compute_ml_tree_only = false;
    this->budget_file = NULL;
    this->overlap = 0;
    this->is_rooted = false;
    this->root_move_dist = 2;
    this->root_find = false;
    this->root_test = false;
    this->sample_size = -1;
    this->repeated_time = 1;
    //this->nr_output = 10000;
    this->nr_output = 0;
    //this->smode = EXHAUSTIVE;
    this->intype = IN_OTHER;
    this->budget = -1;
    this->min_budget = -1;
    this->step_budget = 1;
    this->root = NULL;
    ...
    this->num_zero_len = 0;
    this->pd_limit = 100;
    this->calc_pdgain = false;
    this->multi_tree = false;
    this->second_tree = NULL;
    this->support_tag = NULL;
    this->site_concordance = 0;
    this->ancestral_site_concordance = 0;
    this->site_concordance_partition = false;
    this->print_cf_quartets = false;
    this->print_df1_trees = false;
    this->internode_certainty = 0;
    this->tree_weight_file = NULL;
    this->consensus_type = CT_NONE;
    this->find_pd_min = false;
    this->branch_cluster = 0;
    this->taxa_order_file = NULL;
    this->endemic_pd = false;
    this->exclusive_pd = false;
    this->complement_area = NULL;
    this->scaling_factor = -1;
    this->numeric_precision = -1;
    this->binary_programming = false;
    this->quad_programming = false;
    this->test_input = TEST_NONE;
    this->tree_burnin = 0;
    this->tree_max_count = 1000000;
    this->split_threshold = 0.0;
    this->split_threshold_str = NULL;
    this->split_weight_threshold = -1000;
    this->collapse_zero_branch = false;
    this->split_weight_summary = SW_SUM;
    this->gurobi_format = true;
    this->gurobi_threads = 1;
    this->num_bootstrap_samples = 0;
    this->bootstrap_spec = NULL;
    this->transfer_bootstrap = 0;

    this->aln_file = NULL;
    this->phylip_sequential_format = false;
    this->symtest = SYMTEST_NONE;
    this->symtest_only = false;
    this->symtest_remove = 0;
    this->symtest_keep_zero = false;
    this->symtest_type = 0;
    this->symtest_pcutoff = 0.05;
    this->symtest_stat = false;
    this->symtest_shuffle = 1;
    //this->treeset_file = NULL;
    this->topotest_replicates = 0;
    this->topotest_optimize_model = false;
    this->do_weighted_test = false;
    this->do_au_test = false;
    this->siteLL_file = NULL; //added by MA
    this->partition_file = NULL;
    this->partition_type = BRLEN_OPTIMIZE;
    this->partfinder_rcluster = 100;
    this->partfinder_rcluster_max = 0;
    this->partition_merge = MERGE_NONE;
    this->merge_models = "1";
    this->merge_rates = "1";
    this->partfinder_log_rate = true;
    
    this->sequence_type = NULL;
    this->aln_output = NULL;
    this->aln_site_list = NULL;
    this->aln_output_format = IN_PHYLIP;
    this->output_format = FORMAT_NORMAL;
    this->newick_extended_format = false;
    this->gap_masked_aln = NULL;
    this->concatenate_aln = NULL;
    this->aln_nogaps = false;
    this->aln_no_const_sites = false;
    this->print_aln_info = false;
//    this->parsimony = false;
//    this->parsimony_tree = false;
    this->tree_spr = false;
    this->nexus_output = false;
    this->k_representative = 4;
    this->loglh_epsilon = 0.001;
    this->numSmoothTree = 1;
    this->nni5 = true;
    this->nni5_num_eval = 1;
    this->brlen_num_traversal = 1;
    this->leastSquareBranch = false;
    this->pars_branch_length = false;
    this->bayes_branch_length = false;
    this->manuel_analytic_approx = false;
    this->leastSquareNNI = false;
    this->ls_var_type = OLS;
    this->maxCandidates = 20;
    this->popSize = 5;
    this->p_delete = -1;
    this->min_iterations = -1;
    this->max_iterations = 1000;
    this->num_param_iterations = 100;
    this->stop_condition = SC_UNSUCCESS_ITERATION;
    this->stop_confidence = 0.95;
    this->num_runs = 1;
    this->model_name = "";
    this->model_name_init = NULL;
    this->model_opt_steps = 10;
    this->model_set = "ALL";
    this->model_extra_set = NULL;
    this->model_subset = NULL;
    this->state_freq_set = NULL;
    this->ratehet_set = "AUTO";
    this->score_diff_thres = 10.0;
    this->model_def_file = NULL;
    this->modelomatic = false;
    this->model_test_again = false;
    this->model_test_and_tree = 0;
    this->model_test_separate_rate = false;
    this->optimize_mixmodel_weight = false;
    this->optimize_rate_matrix = false;
    this->store_trans_matrix = false;
    //this->freq_type = FREQ_EMPIRICAL;
    this->freq_type = FREQ_UNKNOWN;
    this->keep_zero_freq = true;
    this->min_state_freq = MIN_FREQUENCY;
    this->min_rate_cats = 2;
    this->num_rate_cats = 4;
    this->max_rate_cats = 10;
    this->gamma_shape = -1.0;
    this->min_gamma_shape = MIN_GAMMA_SHAPE;
    this->gamma_median = false;
    this->p_invar_sites = -1.0;
    this->optimize_model_rate_joint = false;
    this->optimize_by_newton = true;
    this->optimize_alg_freerate = "2-BFGS,EM";
    this->optimize_alg_mixlen = "EM";
    this->optimize_alg_gammai = "EM";
    this->optimize_alg_treeweight = "EM";
    this->optimize_from_given_params = false;
    this->fixed_branch_length = BRLEN_OPTIMIZE;
    this->min_branch_length = 0.0; // this is now adjusted later based on alignment length
    // TODO DS: This seems inappropriate for PoMo.  It is handled in
    // phyloanalysis::2908.
    this->max_branch_length = 10.0; // Nov 22 2016: reduce from 100 to 10!
    this->iqp_assess_quartet = IQP_DISTANCE;
    this->iqp = false;
    this->write_intermediate_trees = 0;
//    this->avoid_duplicated_trees = false;
    this->writeDistImdTrees = false;
    this->rf_dist_mode = 0;
    this->rf_same_pair = false;
    this->normalize_tree_dist = false;
    this->mvh_site_rate = false;
    this->rate_mh_type = true;
    this->discard_saturated_site = false;
    this->mean_rate = 1.0;
    this->aLRT_threshold = 101;
    this->aLRT_replicates = 0;
    this->aLRT_test = false;
    this->aBayes_test = false;
    this->localbp_replicates = 0;
#ifdef __AVX512KNL
    this->SSE = LK_AVX512;
#else
    this->SSE = LK_AVX_FMA;
#endif
    this->lk_safe_scaling = false;
    this->numseq_safe_scaling = 2000;
    this->kernel_nonrev = false;
    this->print_site_lh = WSL_NONE;
    this->print_partition_lh = false;
    this->print_site_prob = WSL_NONE;
    this->print_site_state_freq = WSF_NONE;
    this->print_site_rate = 0;
    this->print_trees_site_posterior = 0;
    this->print_ancestral_sequence = AST_NONE;
    this->min_ancestral_prob = 0.0;
    this->print_tree_lh = false;
    this->lambda = 1;
    this->speed_conf = 1.0;
    this->whtest_simulations = 1000;
    this->mcat_type = MCAT_LOG + MCAT_PATTERN;
    this->rate_file = NULL;
    this->ngs_file = NULL;
    this->ngs_mapped_reads = NULL;
    this->ngs_ignore_gaps = true;
    this->do_pars_multistate = false;
    this->gene_pvalue_file = NULL;
    this->gene_scale_factor = -1;
    this->gene_pvalue_loga = false;
    this->second_align = NULL;
    this->ncbi_taxid = 0;
    this->ncbi_taxon_level = NULL;
    this->ncbi_names_file = NULL;
    this->ncbi_ignore_level = NULL;

	this->eco_dag_file  = NULL;
	this->eco_type = NULL;
	this->eco_detail_file = NULL;
	this->k_percent = 0;
	this->diet_min = 0;
	this->diet_max = 0;
	this->diet_step = 0;
	this->eco_weighted = false;
	this->eco_run = 0;

	this->upper_bound = false;
	this->upper_bound_NNI = false;
	this->upper_bound_frac = 0.0;

    this->gbo_replicates = 0;
	this->ufboot_epsilon = 0.5;
    this->check_gbo_sample_size = 0;
    this->use_rell_method = true;
    this->use_elw_method = false;
    this->use_weighted_bootstrap = false;
    this->use_max_tree_per_bootstrap = true;
    this->max_candidate_trees = 0;
    this->distinct_trees = false;
    this->online_bootstrap = true;
    this->min_correlation = 0.99;
    this->step_iterations = 100;
//    this->store_candidate_trees = false;
	this->print_ufboot_trees = 0;
    this->jackknife_prop = 0.0;
    this->robust_phy_keep = 1.0;
    this->robust_median = false;
    //const double INF_NNI_CUTOFF = -1000000.0;
    this->nni_cutoff = -1000000.0;
    this->estimate_nni_cutoff = false;
    this->nni_sort = false;
    //this->nni_opt_5branches = false;
    this->testNNI = false;
    this->approximate_nni = false;
    this->do_compression = false;

    this->new_heuristic = true;
    this->iteration_multiple = 1;
    this->initPS = 0.5;
#ifdef USING_PLL
    this->pll = true;
#else
...
#endif
    this->modelEps = 0.01;
    this->modelfinder_eps = 0.1;
    this->parbran = false;
    this->binary_aln_file = NULL;
    this->maxtime = 1000000;
    this->reinsert_par = false;
    this->bestStart = true;
    this->snni = true; // turn on sNNI default now
//    this->autostop = true; // turn on auto stopping rule by default now
    this->unsuccess_iteration = 100;
    this->speednni = true; // turn on reduced hill-climbing NNI by default now
    this->numInitTrees = 100;
    this->fixStableSplits = false;
    this->stableSplitThreshold = 0.9;
    this->five_plus_five = false;
    this->memCheck = false;
    this->tabu = false;
    this->adaptPertubation = false;
    this->numSupportTrees = 20;
//    this->sprDist = 20;
    this->sprDist = 6;
    this->sankoff_cost_file = NULL;
    this->numNNITrees = 20;
    this->avh_test = 0;
    this->bootlh_test = 0;
    this->bootlh_partitions = NULL;
    this->site_freq_file = NULL;
    this->tree_freq_file = NULL;
    this->num_threads = 1;
    this->num_threads_max = 10000;
    this->openmp_by_model = false;
    this->model_test_criterion = MTC_BIC;
//    this->model_test_stop_rule = MTC_ALL;
    this->model_test_sample_size = 0;
    this->root_state = NULL;
    this->print_bootaln = false;
    this->print_boot_site_freq = false;
	this->print_subaln = false;
	this->print_partition_info = false;
	this->print_conaln = false;
	this->count_trees = false;
    this->pomo = false;
    this->pomo_random_sampling = false;
	// this->pomo_counts_file_flag = false;
	this->pomo_pop_size = 9;
	this->print_branch_lengths = false;
	this->lh_mem_save = LM_PER_NODE; // auto detect
    this->buffer_mem_save = false;
	this->start_tree = STT_PLL_PARSIMONY;
    this->start_tree_subtype_name = StartTree::Factory::getNameOfDefaultTreeBuilder();

    this->modelfinder_ml_tree = true;
    this->final_model_opt = true;
	this->print_splits_file = false;
    this->print_splits_nex_file = true;
    this->ignore_identical_seqs = true;
    this->write_init_tree = false;
    this->write_candidate_trees = false;
    this->write_branches = false;
    this->freq_const_patterns = NULL;
    this->no_rescale_gamma_invar = false;
    this->compute_seq_identity_along_tree = false;
    this->compute_seq_composition = true;
    this->lmap_num_quartets = -1;
    this->lmap_cluster_file = NULL;
    this->print_lmap_quartet_lh = false;
    this->num_mixlen = 1;
    this->link_alpha = false;
    this->link_model = false;
    this->model_joint = NULL;
    this->ignore_checkpoint = false;
    this->checkpoint_dump_interval = 60;
    this->force_unfinished = false;
    this->print_all_checkpoints = false;
    this->suppress_output_flags = 0;
    this->ufboot2corr = false;
    this->u2c_nni5 = false;
    this->date_with_outgroup = true;
    this->date_debug = false;
    this->date_replicates = 0;
    this->clock_stddev = -1.0;
    this->date_outlier = -1.0;
    
    // ------------ Terrace variables ------------
    this->terrace_check = false;
    this->terrace_analysis = false;
    this->print_terrace_trees = false;
    this->print_induced_trees = false;
    this->pr_ab_matrix = nullptr;
    this->print_pr_ab_matrix = false;
    this->print_m_overlap = false;
    this->terrace_query_set = nullptr;
    this->terrace_stop_intermediate_num = -1;
    this->terrace_stop_terrace_trees_num = -1;
    this->terrace_stop_time = -1;
    this->terrace_non_stop = false;
    this->terrace_print_lim = 0;
    this->terrace_remove_m_leaves = 0;
    this->matrix_order = false;
    this->gen_all_NNI = false;
    
    this->remove_empty_seq = true;
    this->terrace_aware = true;
#ifdef IQTREE_TERRAPHAST
    this->terrace_analysis_tphast = false;
#else
    this->terrace_analysis_tphast = false;
#endif
    ...
    this->matrix_exp_technique = MET_EIGEN3LIB_DECOMPOSITION;
    ...
    gettimeofday(&tv, &tz);
    //this->ran_seed = (unsigned) (tv.tv_sec+tv.tv_usec);
    this->ran_seed = (tv.tv_usec);
    this->subsampling_seed = this->ran_seed;
    this->subsampling = 0;
    
    this->suppress_list_of_sequences = false;
    this->suppress_zero_distance_warnings = false;
    this->suppress_duplicate_sequence_warnings = false;
    
    this->original_params = "";
    this->alisim_active = false;
    this->alisim_inference_mode = false;
    this->alisim_no_copy_gaps = false;
    this->alisim_sequence_length = 1000;
    this->alisim_dataset_num = 1;
    this->alisim_ancestral_sequence_aln_filepath = NULL;
    this->alisim_ancestral_sequence_name = "";
    this->alisim_max_rate_categories_for_applying_caching = 100;
    this->alisim_num_states_morph = 0;
    this->alisim_num_taxa_uniform_start = -1;
    this->alisim_num_taxa_uniform_end = -1;
    this->alisim_length_ratio = 2;
    this->birth_rate = 0.8;
    this->death_rate = 0.2;
    this->alisim_fundi_proportion = 0;
    this->alisim_distribution_definitions = NULL;
    this->alisim_skip_checking_memory = false;
    this->alisim_write_internal_sequences = false;
    this->alisim_only_unroot_tree = false;
    this->branch_distribution = NULL;
    this->alisim_insertion_ratio = 0;
    this->alisim_deletion_ratio = 0;
    this->alisim_insertion_distribution = IndelDistribution(ZIPF,1.7,100);
    this->alisim_deletion_distribution = IndelDistribution(ZIPF,1.7,100);
    this->alisim_mean_deletion_size = -1;
    this->alisim_simulation_thresh = 0.001;
    this->delay_msgs = "";
    this->alisim_no_export_sequence_wo_gaps = false;
    this->alisim_mixture_at_sub_level = false;
    this->alisim_branch_scale = 1.0;
    this->alisim_rate_heterogeneity = POSTERIOR_MEAN;
    this->alisim_stationarity_heterogeneity = POSTERIOR_MEAN;
    this->alisim_single_output = false;
    this->keep_seq_order = false;
    this->mem_limit_factor = 0;
    this->delete_output = false;
    this->indel_rate_variation = false;
    this->tmp_data_filename = "tmp_data";
    this->rebuild_indel_history_param = 1.0/3;
    this->alisim_openmp_alg = IM;
    this->no_merge = false;
    this->alignment_id = 0;
    this->include_pre_mutations = false;
    this->mutation_file = "";
    this->site_starting_index = 0;
