...
def test_param_manipulation():
    ...
    assert "fai" in initial_params, "get_params: Parameter 'fai' not found in initial parameters."

    original_value = initial_params["fai"]
    piqtree2.set_params({"fai": not original_value})
    updated_value = piqtree2.get_param("fai")
    assert updated_value != original_value, "set_params: Parameter 'fai' did not update correctly."

def test_set_unknown_param():
    with pytest.raises(ValueError) as excinfo:
        piqtree2.set_params({"unknown_param": True})
    assert "unknown parameter" in str(excinfo.value), "set_params: No error raised for unknown parameter."

def test_get_unknown_param():
    result = piqtree2.get_param("unknown_param")
    assert result is None, "get_param: Expected None for unknown parameter, got {}".format(result)

def test_set_invalid_param_type():
    with pytest.raises(TypeError) as excinfo:
        piqtree2.set_params({"fai": "not a boolean"})
    assert "Incorrect type" in str(excinfo.value), "set_params: No error raised for incorrect parameter type."
